	...
	public function setDepth($model){
		try {
			if($model->getOrderItem()){
				...
				if($model->getPositionItem() == \Commercepundit\MegaMenu\Model\Config\Source\PositionItem::AFTER){		//after item:
					$depth =  intval($data['depth']);
				}
				elseif($model->getPositionItem() == \Commercepundit\MegaMenu\Model\Config\Source\PositionItem::BEFORE){		//before item:
					$depth =  intval($data['depth']);
				}
			}
			else{
