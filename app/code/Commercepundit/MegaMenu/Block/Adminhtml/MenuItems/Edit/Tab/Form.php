<?php
namespace Commercepundit\MegaMenu\Block\Adminhtml\MenuItems\Edit\Tab;
	...
	\Magento\Backend\Block\Widget\Tab\TabInterface
{
	...
	{
		return $this->_objectManager->create('Commercepundit\MegaMenu\Model\ResourceModel\MenuGroup\Collection');
	}
	...
	protected function _prepareForm()
	{
		...
		$fieldset->addField(
			...
			[
				...
				'name' => 'status',
				'values' => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\Status')->toOptionArray(),
				'disabled' => $isElementDisabled
			]
		);
		...
		$fieldset->addField(
			...
			[
				...
				'title' => __('Show Title'),
				'name' => 'show_title',
				'values' => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\YesNo')->toOptionArray(),
				'disabled' => $isElementDisabled
			]
		);
		...
		$fieldset->addField(
			...
			[
				...
				'title' => __('Align'),
				'name' => 'align',
				'values' => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\Align')->toOptionArray(),
				'disabled' => $isElementDisabled
			]
		);
		...
		$fieldset->addField(
			...
			[
				...
				'name' => 'position_item',
				'values' => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\PositionItem')->toOptionArray(),
				'disabled' => $isElementDisabled
			]
		);
		...
		$col = $fieldset->addField(
			...
			[
				...
				'name' => 'cols_nb',
				'values' => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\ListNumCol')->toOptionArray(),
				'disabled' => $isElementDisabled
			]
		);
		...
		$fieldset->addField(
			...
			[
				...
				'name'      => 'target',
				'values'    => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\LinkTargets')->toOptionArray(),
			]
		);
		...
		$type = $fieldset->addField(
			...
			[
				...
				'title'     => __('Menu Type'),
				'name'      => 'type',
				'values'    => $objectManager->get('Commercepundit\MegaMenu\Model\Config\Source\Type')->toOptionArray(),
		...
		$addWidget = $this->_blockFactory->createBlock('\Commercepundit\MegaMenu\Block\Adminhtml\Widget\AddField');
	...
	/*	$textarea = $fieldset->addField(
		...
		$type->setAfterElementHtml('
					<script type="text/javascript">
						...
						function CheckType(element){
							...
							if(type=='.\Commercepundit\MegaMenu\Model\Config\Source\Type::CONTENT.'){
								/*$$("[id^=\''.'content\']").each(function(element){
							...
							else if(type!='.\Commercepundit\MegaMenu\Model\Config\Source\Type::NORMAL.'){
								$$(".data_type").each(function(element){
									element.up().up().show();
									element.addClassName("required-entry");
								});
							}
							if(type=='.\Commercepundit\MegaMenu\Model\Config\Source\Type::PRODUCT.'){
								$$("div[id^=\''.'box_product_id\']")[0].up().up().up().show();
							}
							if(type=='.\Commercepundit\MegaMenu\Model\Config\Source\Type::CATEGORY.'){
								$$("div[id^=\''.'box_category_id\']")[0].up().up().up().show();
							}
							if(type=='.\Commercepundit\MegaMenu\Model\Config\Source\Type::CMSPAGE.'){
								$$("div[id^=\''.'box_page_id\']")[0].up().up().up().show();
							}
							if(type=='.\Commercepundit\MegaMenu\Model\Config\Source\Type::CMSBLOCK.'){
		...
		$renderer = $this->getLayout()->createBlock(
			'Commercepundit\MegaMenu\Block\Adminhtml\Widget\Form\Renderer\SortAble'
		);
