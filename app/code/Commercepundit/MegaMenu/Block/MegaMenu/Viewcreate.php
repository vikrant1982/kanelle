<?php
namespace Commercepundit\MegaMenu\Block\MegaMenu;
...
class Viewcreate extends Template
{
	/**
	...
	const EXTERNALLINK = \Commercepundit\MegaMenu\Model\Config\Source\Type::EXTERNALLINK;
	const PRODUCT = \Commercepundit\MegaMenu\Model\Config\Source\Type::PRODUCT;
	const CATEGORY = \Commercepundit\MegaMenu\Model\Config\Source\Type::CATEGORY;
	const CMSBLOCK = \Commercepundit\MegaMenu\Model\Config\Source\Type::CMSBLOCK;
	const CMSPAGE = \Commercepundit\MegaMenu\Model\Config\Source\Type::CMSPAGE;
	const CONTENT = \Commercepundit\MegaMenu\Model\Config\Source\Type::CONTENT;
	const STATUS_ENABLED = \Commercepundit\MegaMenu\Model\Config\Source\Status::STATUS_ENABLED;
	const PREFIX = \Commercepundit\MegaMenu\Model\Config\Source\Html::PREFIX;
	...
	)
	{
		...
		$this->addData([
			...
			'cache_tags'     => ['cp_megamenucreate'],
			'cache_key'      => 'gropus200'.$this->_defaults['group_id'],
		]);
	}
	...
	protected function _toHtml()
	{
		...
		$Cache_Lite = new \Commercepundit\MegaMenu\Block\Cache\Lite($options);
		...
		if ($use_cache){
			...
			if ($data = $Cache_Lite->get($hash)) {
				...
				$template_file = (!empty($template_file)) ? $template_file : "Commercepundit_MegaMenu::megamenucreate.phtml";
	...
	public function nameTable(){
		return $this->_objectManager->create('Commercepundit\MegaMenu\Model\ResourceModel\MenuItems')->getMainTable();
	}
	...
	public function getItems()
	{
		...
		$group_item = $menuGroup->load($this->getData('group_id'));
		if($group_item->getStatus() == self::STATUS_ENABLED){
			$collection_items = $this->_menuItem->getItemsByLv($this->getData('group_id'), $this->_defaults['start_level']);
	...
	public function isAlignRight($item){
		return ($item['align']==\Commercepundit\MegaMenu\Model\Config\Source\Align::RIGHT)?true:false;
	}
	...
	public function isDrop($item){
		// return ($item['show_as_group']==\Commercepundit\MegaMenu\Model\Config\Source\Status::STATUS_DISABLED)?true:false;
		return (1 == \Commercepundit\MegaMenu\Model\Config\Source\Status::STATUS_DISABLED)?true:false;
	}
	...
	public function getItemHtml($item, $isFirstColumn ='', $idActive = ''){
		...
		if($item['align'] && $item['align'] == \Commercepundit\MegaMenu\Model\Config\Source\Align::RIGHT){
...
//								return $html;
     ...
     * @param string $type
     * @param string $folder
     * @return \Magento\Framework\Image\Adapter\AdapterInterface|string
     */
	...
	public function getCategory($item, $itemId){
		...
		if ($category->getChildren() && $item['show_sub_category'] == self::STATUS_ENABLED)
		{
	...
	public function getCurrentUrl()
	 {
	  $current = explode('?',$this->_urlinterface->getCurrentUrl());
	  $linkCur = (isset($current) && (count($current) >=2)) ? $current[0] : $this->_urlinterface->getCurrentUrl();
	  return $linkCur;
	 }
}